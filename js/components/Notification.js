...
const Notification = ({ message, type = 'success', duration = 3000, onClose, className = '' }) => {
    ...
    return html`
        <div class="absolute inset-0 px-4 z-[9999] transition-all duration-300 ease-in-out flex items-center justify-center text-center bg-gradient-to-br from-white/40 to-white/10 dark:from-black/40 dark:to-black/10 backdrop-blur-xl border border-white/30 dark:border-white/10 shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] ring-1 ring-white/40 dark:ring-white/10 ${className}"
             style="opacity: ${isVisible ? 1 : 0};">
            <span class="text-white text-sm font-medium transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'}">${message}</span>
        </div>
    `;
};
