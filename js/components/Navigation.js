...
const Navigation = ({ active, onChange, class: additionalClass, isNavbarHidden }) => {
    ...
    return html`
        <nav class="absolute bottom-6 left-6 right-6 h-16 rounded-full px-4 z-50 transition-all duration-300
            bg-gradient-to-br from-white/40 to-white/10 dark:from-black/40 dark:to-black/10 backdrop-blur-xl border border-white/30 dark:border-white/10 shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] ring-1 ring-white/40 dark:ring-white/10 ${additionalClass}"
        >
            <div class="relative w-full h-full transition-opacity duration-300 ${isNavbarHidden ? 'opacity-0' : 'opacity-100'}">
            ...
            ${notification && html`<${Notification} message=${notification.message} type=${notification.type} duration=${notification.duration} onClose=${hideNotification} className="h-16 rounded-full" />`}
        </nav>
    `;
};
